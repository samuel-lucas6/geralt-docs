...
3 and 4 can be resolved by using a [counter](../constant-time.md#increment) nonce (the initial nonce can be [random](../random-data.md#fill)) or by including the previous tag in the associated data of the next chunk.
...
{% hint style="info" %}
Consider [ChaCha20-Poly1305](chacha20-poly1305.md) if you do not need random nonces with the same key. For example, if you can use a [counter](../constant-time.md#increment) nonce or rotate the key for each encryption operation. It is marginally more efficient.
{% endhint %}
